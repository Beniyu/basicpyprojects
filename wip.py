...
def pop_up(type_of_message):
    ...
    if type_of_message=='incorrect':
        message_pop_up.config(text='Incorrect',background='red',foreground='white',font=('Times','340'),pady=180,padx=42)
    message_pop_up.pack(fill='both',expand='yes')
    pop_up.after(1000,lambda: pop_up.destroy())
    time1=time.clock()
    while time.clock()-time1>1:
        print('lol')
...
def arithmetic_question():
    typeofproblem=r.choice(['times','plus','minus'])
...
total_attempts=-1
computer_answer=0
operations={'times':{'display':'x','real':'*','boundary':[1,12]},
    'plus':{'display':'+','real':'+','boundary':[1,100]},
    'minus':{'display':'-','real':'-','boundary':[1,100]}}
root=tk.Tk()
root.title('Arithmetic program')
questionWindow=tk.Label(root,text="Welcome!",relief='raised')
questionWindow.pack()
user_input=tk.StringVar()
answerWindow=tk.Entry(root,textvariable=user_input)
answerWindow.pack()
buttonFrame=tk.Frame(root)
buttonFrame.pack(fill='x',expand='yes')
tk.Button(buttonFrame,text='Confirm',command=lambda: confirm(user_input.get())).pack(side='left',fill='x',expand='yes')
tk.Button(buttonFrame,text='End',command=finish_program).pack(side='right',expand='no')
